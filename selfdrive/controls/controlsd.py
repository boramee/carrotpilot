...
class Controls:
  ...
  def state_control(self):
    ...
    # Pitch (orientationNED[1]) is used for adaptive soft-stop on grades.
    pitch = float(self.calibrated_pose.orientation.xyz[1]) if self.calibrated_pose is not None else 0.0
    accel, aTarget, jerk = self.LoC.update(CC.longActive, CS, long_plan, pid_accel_limits, t_since_plan, self.sm['radarState'], pitch)
